function [y]=lpfilt(x,dt,fc,n,type);
% FUNCTION [Y]=LPFILT(X,DT,FC,N,TYPE);
%  LPFILT Lowpass Butterworth filter.
%         LPFILT(X,DT,FC,N,TYPE) takes a time series sampled at DT 
%         and filters it with an Nth order, butterworth filter 
%         with cutoff frequency FC (in Hz). TYPE=0 specifies
...
% Amplitude spectrum.
Fl(1:n2/2+1)=sqrt(1./(1+(omega/omegac).^(2*n)));
